...
-- 202609160002 qualified two of the three bare trip_id references inside
-- join_trip_via_invite but missed the referrer-validation check, which is
-- still ambiguous against the function's implicit `trip_id` OUT variable
-- (from `returns table (trip_id uuid, ...)`). PL/pgSQL parses/plans the
-- whole IF condition up front, so this fired on every join -- including
-- manual invite-code entry with no referrer, since p_referrer_user_id being
-- null only short-circuits at runtime, after the ambiguous parse already failed.
